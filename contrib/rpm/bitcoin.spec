...
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 18326
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 18327
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 35351
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 19837
...
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	%{_sbindir}/semanage port -d -p tcp 18326
	%{_sbindir}/semanage port -d -p tcp 18327
	%{_sbindir}/semanage port -d -p tcp 35351
	%{_sbindir}/semanage port -d -p tcp 19837
	for selinuxvariant in %{selinux_variants}; do
